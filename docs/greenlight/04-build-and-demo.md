...
- **2:00–2:40 — The competent payoff, shown fast.** Allocation donut animates in (ESG-screened, age-tilted). The **Monte Carlo fan chart** with the live success-% headline (≈80% range, illustrative — quote whatever the seeded run produces, don't hardcode "82%") + the Gaussian left-tail contrast. Then the **pre-computed backtest** beating 60/40 with the **Deflated Sharpe** visible. "Risk-parity, no fragile forecasts, tested out-of-sample with overfitting-aware metrics."
