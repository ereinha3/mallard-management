...
| 7 | [07-elicitation-and-fusion.md](./07-elicitation-and-fusion.md) | **Risk elicitation flow & signal fusion.** How the LLM conversationally administers the validated instruments (Grable-Lytton 13 + Dohmen single-item + loss-aversion probe) and the exact `RiskSignals` contract it emits; how the engine fuses them into the γ band via inverse-variance / random-effects pooling. **The hand-off doc for the LLM-integration pipeline.** |
| — | [data-bom.md](./data-bom.md) | **Data bill of materials.** Every dataset the engine needs from free sources (yfinance + FRED), in the schemas the loaders ingest. |
