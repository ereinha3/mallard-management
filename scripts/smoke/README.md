...
| `alpaca_broker_smoke.py` | Live Alpaca Broker API sandbox flow (create account → ACH relationship → deposit → buy target portfolio → read positions) | `BROKER_API_KEY` and `BROKER_SECRET_KEY` in `backend/.env` |
